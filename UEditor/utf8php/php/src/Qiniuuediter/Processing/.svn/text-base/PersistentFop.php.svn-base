...
final class PersistentFop
{
    ...
    public function execute($key, $fops)
    {
        ...
        \Qiniuuediter\setWithoutEmpty($params, 'pipeline', $this->pipeline);
        \Qiniuuediter\setWithoutEmpty($params, 'notifyURL', $this->notify_url);
        if ($this->force) {
            $params['force'] = 1;
        }
