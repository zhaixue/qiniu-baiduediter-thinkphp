<?php
namespace Qiniuuediter\Storage;

use Qiniuuediter\Auth;
use Qiniuuediter\Config;
use Qiniuuediter\Http\Client;
use Qiniuuediter\Http\Error;
...
final class BucketManager
{
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        \Qiniuuediter\setWithoutEmpty($query, 'prefix', $prefix);
        \Qiniuuediter\setWithoutEmpty($query, 'marker', $marker);
        \Qiniuuediter\setWithoutEmpty($query, 'limit', $limit);
        \Qiniuuediter\setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    {
        $path = '/stat/' . \Qiniuuediter\entry($bucket, $key);
    ...
    {
        $path = '/delete/' . \Qiniuuediter\entry($bucket, $key);
    ...
    {
        $from = \Qiniuuediter\entry($from_bucket, $from_key);
        $to = \Qiniuuediter\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list(, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    {
        $resource = \Qiniuuediter\entry($bucket, $key);
        $encode_mime = \Qiniuuediter\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \Qiniuuediter\base64_urlSafeEncode($url);
        $to = \Qiniuuediter\entry($bucket, $key);
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \Qiniuuediter\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \Qiniuuediter\entry($source_bucket, $from_key);
            $to = \Qiniuuediter\entry($target_bucket, $to_key);
